...
/*
    Author: Koushik Sahu
    Created: 05:32:54 PM(17:32:54) IST(+05:30) 28-03-2026 Sat
*/

fn solve<R: BufRead, W: Write>(sc: &mut Scanner<R>, wr: &mut W) {
    let (n, m) = (sc.next::<i32>(), sc.next::<i32>());
    let mut members = vec![0; m as usize];
    for _ in 0..n {
        let (a, b) = (sc.next::<i32>(), sc.next::<i32>());
        members[a as usize - 1] -= 1;
        members[b as usize - 1] += 1;
    }
    for i in 0..m {
        write!(wr, "{} ", members[i as usize]).unwrap();
    }
    writeln!(wr).unwrap();
}
