...
fn possible(s: String, opts: &Vec<String>, dp: &mut BTreeMap<String, bool>) -> bool {
    if s.len() == 0 {
        return true;
    }
    ...
    for opt in opts.iter() {
        if s.len() >= opt.len() {
            if &s[..opt.len()] == *opt {
                ok |= possible(s[opt.len()..].to_owned(), opts, dp);
            }
        }
    }
    dp.insert(s, ok);
...
fn solve<R: BufRead, W: Write>(sc: &mut Scanner<R>, wr: &mut W) {
    ...
    let mut ans = 0;
    let mut dp = BTreeMap::<String, bool>::new();
    for p in patterns {
        if possible(p, &opts, &mut dp) {
            ans += 1;
        }
    }
