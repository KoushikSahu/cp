...
/*
    Author: Koushik Sahu
    Created: 08:15:24 PM(20:15:24) IST(+05:30) 07-03-2026 Sat
*/

fn solve<R: BufRead, W: Write>(sc: &mut Scanner<R>, wr: &mut W) {
    let (n, k) = (sc.next::<i32>(), sc.next::<i32>());
    let max_bit = (n - 1).count_ones();
    let mut ans = 0;
    let mut ncr = vec![vec![0; 31]; 31];
    for i in 0..31 {
        for j in 0..31 {
            if i == 0 && j == 0 {
                ncr[i][j] = 1;
    ...
    for i in 1..=max_bit {
        for j in 0..i {
            if i as i32 + j as i32 > k {
                let cnt = ncr[i as usize - 1][j as usize];
                ans += cnt;
            }
        }
    }
    if max_bit as i32 + 1 > k {
        ans += 1;
    }
